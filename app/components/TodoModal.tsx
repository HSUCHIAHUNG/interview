...
export default function TodoModal({ onClose }: Props) {
    ...
    update(
      sections.map((s) =>
        s.id === sId
          ? {
              ...s,
              items: [...s.items, { id: genId(), text: "", done: false }],
            }
          : s
      )
    );
  ...
  const statusText =
    status === "saving"
      ? "儲存中..."
      : status === "saved"
      ? `已儲存 ${savedAt ? formatTime(savedAt) : ""}`
      : status === "error"
      ? "儲存失敗"
      : dirty
      ? "有未儲存的變更"
      : savedAt
      ? `上次儲存 ${formatTime(savedAt)}`
      : "";
          ...
          {loading ? (
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-20 rounded-lg bg-gray-800/50 animate-pulse"
                />
              ))}
            </div>
          ) : sections.length === 0 ? (
            ...
            sections.map((section) => (
              <SectionBlock
                ...
                onUpdateItem={(iId, patch) =>
                  updateItem(section.id, iId, patch)
                }
...
function ItemRow({ item, onUpdate, onDelete, onAddNext }: ItemRowProps) {
